...
def BCE_loss(Y, X):
    msk =X.clone().detach() 
    msk[msk <1e-6]=1 #just to check convergance
    msk[msk >1e-6]=1


    halfmsk=Y.clone().detach() 
    halfmsk[halfmsk!=0.5]=1
    halfmsk[halfmsk==0.5]=0.1

    Clambda=(2*torch.atan(1-2*Y*halfmsk))/(1-2*Y*halfmsk)

    halfmsk=Y.clone().detach() 
    halfmsk[halfmsk!=0.5]=1
    halfmsk[halfmsk==0.5]=0
    ...
    Clambda=torch.log(halfmsk*2+Clambda)
    
    ret=F.binary_cross_entropy(Y, X, reduction='none')
    ret = (ret)*msk
    ret = torch.sum(ret,1)
    return ret 
...
def BMSE_loss(Y, X, beta,sigma,Dim):
    term1 = -math.log(((1+beta) / beta))
    K1=(math.log(2*math.pi *( sigma** 2)))*(beta*Dim/2)
    term2=MSE_loss(Y, X)
    term3=(beta/(2*( sigma** 2)))*term2
