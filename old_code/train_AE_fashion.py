...
CODE_SIZE = 20  # was 9
SIGMA = 0.5  # for Gaussian Loss function
...
def train(epoch, beta_val):
    ...
    for batch_idx, (data, data_lab) in enumerate(train_loader):
        ...
        loss = beta_loss_function(recon_batch, data,beta_val,mu)+regularize_loss*reg_weight
        ...
        for i, (data, data_lab) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                           'results/fashion_rae_mnist_recon_shallow_' +
                           str(beta_val) + '_' + str(frac_anom) + '.png',
                           nrow=n)

        np.savez('results/fashion_rae_mnist_' + str(beta_val) + '_' +
                 str(frac_anom) + '.npz',
